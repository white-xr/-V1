...
from src.detect.crosswalk_person_detect import CrosswalkPersonDetector
...
class PDMainWindow(QMainWindow, ui_mainwindow.Ui_MainWindow):
    def __init__(self):
        ...
        self.people_detect = CrosswalkPersonDetector()
    ...
    def set_user_info(self, username, role):
        ...
        self.user_role = role
        self.statusbar.showMessage(f'当前用户: {username} ({role})')
    ...
    def setup_permissions(self):
        ...
        # 设置管理员特有功能的可见性
        if hasattr(self, 'actionbeginRecoding'):
            self.actionbeginRecoding.setEnabled(is_admin)
        if hasattr(self, 'actionendRecoding'):
            self.actionendRecoding.setEnabled(is_admin)
    ...
    def output_text(self, text):
        ...
        new_text = current_text + timestamp + " " + text + "\n"
    ...
    def slotUpdateResult(self, original_frame, detected_frame, count):
        ...
        # 提取检测结果信息
        detection_summary = f"检测到人数: {count} 个行人"
    ...
    def slotOpenInstructions(self):
        QMessageBox.information(self, '使用说明', '请查看程序安装目录中"操作文档",如有问题,先问AI再联系作者', QMessageBox.Yes)

    def slotPauseVideo(self):
        # 检查视频线程
        if self.videoReadThread.isRunning():
            self.videoReadThread.pause = not self.videoReadThread.pause
            button_text = "继续" if self.videoReadThread.pause else "暂停"
            self.pauseButton.setText(button_text)
            status_text = "视频已暂停" if self.videoReadThread.pause else "视频已继续播放"
            self.statusbar.showMessage(status_text)
        # 检查摄像头线程
        elif self.cameraReadThread.isRunning():
            self.cameraReadThread.pause = not self.cameraReadThread.pause
            button_text = "继续" if self.cameraReadThread.pause else "暂停"
            self.pauseButton.setText(button_text)
            status_text = "摄像头已暂停" if self.cameraReadThread.pause else "摄像头已继续播放"
            self.statusbar.showMessage(status_text)
        else:
            self.statusbar.showMessage("没有正在播放的视频或摄像头")
